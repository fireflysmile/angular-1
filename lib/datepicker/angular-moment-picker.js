		...
		MomentPickerDirective.prototype.link = function ($scope, $element, $attrs) {
			$scope.template = (
				'<div class="moment-picker-container {{view.selected}}-view" ' +
					...
					'<p class="title-date-set">' +
						'<span class="text-date one">' +
							'From Date' +
						'</span>' +
						'<span class="text-date two">' +
							'Date To' +
						'</span>' +
						'<span class="icon-page icon-close-date" ' +
					...
					'<table class="header-view">' +
						'<thead>' +
							'<tr>' +
								'<th ng-class="{disabled: !view.previous.selectable}" ng-click="view.previous.set()"><span class="pull-left rank-click fix"></span></th>' +
								'<th ng-bind="view.title" ng-click="view.setParentView()"></th>' +
								'<th ng-class="{disabled: !view.next.selectable}" ng-click="view.next.set()"><span class="pull-left rank-click rotate-180 fix"></span></th>' +
							'</tr>' +
						'</thead>' +
					'</table>' +
							...
							'<tbody>' +
								'<tr ng-repeat="fourYear in decadeView.fourYears">' +
									'<td ng-repeat="year in fourYear track by year.year" ' +
										'ng-class="year.class" ng-click="decadeView.setYear(year)"><span ng-bind="year.label"></span></td>' +
								'</tr>' +
							'</tbody>' +
						'</table>' +
							...
							'<tbody>' +
								'<tr ng-repeat="fourMonth in yearView.fourMonths">' +
									'<td ng-repeat="month in fourMonth track by month.month" ' +
										'ng-class="month.class" ng-click="yearView.setMonth(month)"><span ng-bind="month.label"></span></td>' +
								'</tr>' +
							'</tbody>' +
						'</table>' +
						'<table ng-if="view.selected == \'month\'">' +
							...
							'<tbody>' +
								'<tr ng-repeat="days in monthView.weeks">' +
									'<td ng-repeat="day in days track by day.date" ng-class="day.class" ng-click="monthView.setDay(day)"><span ng-bind="day.label"></span></td>' +
								'</tr>' +
							'</tbody>' +
						'</table>' +
							...
							'<tbody>' +
								'<tr ng-repeat="threeHours in dayView.threeHours">' +
									'<td ng-repeat="hour in threeHours track by hour.hour" ' +
										'ng-class="hour.class"><span ng-bind="hour.label" ng-click="dayView.setHour(hour)"></span></td>' +
								'</tr>' +
							'</tbody>' +
						'</table>' +
			...
			$scope.input
				...
				.on('blur',    function () { $timeout(function () { if ($scope.view.isOpen && document.activeElement !== $scope.input[0]) $scope.view.close(); }, 250); })
